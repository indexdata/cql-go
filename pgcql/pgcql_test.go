...
func TestParsing(t *testing.T) {
	...
	for _, testcase := range []struct {
		...
		{"full = \"abc\"", "to_tsvector('english', full) @@ to_tsquery('english', $1)", []any{"'abc'"}},
		{"full = \"abc \"", "to_tsvector('english', full) @@ to_tsquery('english', $1)", []any{"'abc'"}},
		{"full adj \"a* b\"", "error: masking op * unsupported", nil},
		...
		{"full adj \"a b\" c", "to_tsvector('english', full) @@ to_tsquery('english', $1) AND to_tsvector('english', full) @@ to_tsquery('english', $2)", []any{"'a'<->'b'", "'c'"}},
